...
TEST(vector_3s_test, squared_euclidean_norm)
{
	vector_3s x;
	
	//
	x = idlib::zero<vector_3s>();
	ASSERT_EQ(idlib::squared_euclidean_norm(x), idlib::zero<scalar>());
	
	//
    x = vector_3s::unit(0); 
    ASSERT_EQ(idlib::squared_euclidean_norm(x), idlib::one<scalar>());
