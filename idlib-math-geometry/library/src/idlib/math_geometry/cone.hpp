...
/// @file idlib/math_geometry/cone.hpp
...
/**
 * @brief A single infinite cone consists of an origin point \f$O\f$, an axis ray whose origin is at
 * \f$O\f$ and unit length direction is \f$\hat{d}\f$, and and an acute angle \f$\theta\in\left
 * (0, 90\right)\f$.
 * @remark
 * A point \$P\f$ is inside a cone if the angle between the axis ray \f$\hat{d}\f$ and \f$P - O\f$ is
 * in \f$[0,\theta)\f$ and one a cone if that angle is exactly 180.
 *
 ...
 * Rewriting the equation, one obtains
 * \f{align*}{
