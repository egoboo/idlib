...
public:
    ...
    /// @remark
    /// The text is empty.
    /// The trim policy is idlib::document_trim_policy::trailing.
    ...
    signal<void(const text_erased_event&)> text_erased_signal;
    
    /// @brief Signal raised if text was inserted.
    signal<void(const text_inserted_event&)> text_inserted_signal;
    
    /// @brief Signal raised if text was replaced.
    signal<void(const text_replaced_event&)> text_replaced_signal;

    /// @brief Signal raised if text was set.
    signal<void(const text_set_event&)> text_set_signal;

    /// @brief Erase a subtext.
    /// @param index the index of the subtext
    /// @except idlib::out_of_bounds_error @a index is not within the bounds of @a 0 and @a text.get_size().
    ...
    /// @except idlib::out_of_bounds_error @a index is not within the bounds of @a 0 and @a text.get_size().
    /// @except idlib::out_of_bounds_error @a (index + length) is not within the bounds of @a 0 and @a text.get_size().
    ...
    /// @throw idlib::out_of_bounds_error @a indx is greater than the length of the text
    ...
    idlib::document_trim_policy get_trim_policy() const;

    /// @brief Set the trim policy of this document.
    /// @param trim_policy the trim policy of this document
    void set_trim_policy(idlib::document_trim_policy trim_policy);
