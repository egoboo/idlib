...
#include "idlib/hll/category_element.hpp"

#define IDLIB_HLL_TOKEN_WITH_ENDLOCATION (0)

namespace idlib::hll {

/// @brief Generic token.
/// @tparam Kind the type of the kinds of this token type.
template <typename Kind, Kind Default_Kind>
class token : public category_element<Kind, Default_Kind>
{
private:
    /// @brief The start location of this token.
    location m_start_location;

#if defined(IDLIB_HLL_TOKEN_WITH_END_LOCATION) && 1 == IDLIB_HLL_TOKEN_WITH_END_LOCATION
...
public:
    ...
    token(Kind kind,
        ...
        : category_element<Kind, Default_Kind>(kind),
