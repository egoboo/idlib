...
/// @file idlib/exception/argument_out_of_bounds_error.hpp
...
class argument_out_of_bounds_error : public invalid_argument_error
{
public:
    ...
    explicit argument_out_of_bounds_error(const char *file, int line, const std::string& name) :
        invalid_argument_error(file, line, "argument `" + name + "` is out of bounds")
    {}

    friend void swap(argument_out_of_bounds_error& a, argument_out_of_bounds_error& b)
    {
        using std::swap;
...
}; // class argument_out_of_bounds_error
