...
{
    auto p = idlib::parsing_expression::sym<char>('.');
    std::string w;
...
{
    auto p = idlib::parsing_expression::whitespace<char>();
    std::string w;
    std::string::const_iterator s, e;
    //
    w = std::string(" ");
...
{
    auto p = idlib::parsing_expression::newline<char>();
    std::string w;
    std::string::const_iterator s, e;
    //
    w = std::string("\n");
...
{
    auto p = idlib::parsing_expression::digit<char>();
...
{
    auto p = idlib::parsing_expression::alpha_lowercase<char>();
...
{
    auto p = idlib::parsing_expression::alpha_uppercase<char>();
    for (char symbol = 'A'; symbol <= 'Z'; ++symbol)
    {
        auto w = std::string(1, symbol);
        auto s = w.cbegin(), e = w.cend();
...
TEST(single_symbol_testing, test_single_symbol)
{
    ...
    for (char symbol = 'a'; symbol <= 'z'; ++symbol)
    {
        auto p = idlib::parsing_expression::sym(symbol);
        auto w = std::string(1, symbol);
        auto s = w.cbegin(), e = w.cend();
        ASSERT_TRUE(p(s, e));
        ASSERT_EQ(p(s, e).range().end(), e);
...
{
    auto p = idlib::parsing_expression::alpha<char>();
    //
    for (char symbol = 'a'; symbol <= 'z'; ++symbol)
    {
        auto w = std::string(1, symbol);
